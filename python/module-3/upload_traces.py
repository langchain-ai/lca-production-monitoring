...
Compared to the module-2 version, this script also remaps thread_id values
in extra.metadata so that threads remain consistent after ID regeneration.
"""
...
def main():
    ...
    parser.add_argument("--project", default="ACME-quality", help="Target project name")
    parser.add_argument("--input", default="module3_traces.json", help="Input file path")
    ...
    # Build thread_id map (preserve grouping with new UUIDs)
    thread_map: dict[str, str] = {}
    for run in runs:
        md = (run.get("extra") or {}).get("metadata", {})
        old_thread = md.get("thread_id")
        if old_thread and old_thread not in thread_map:
            thread_map[old_thread] = str(uuid7())
    ...
    for run in runs:
        ...
        # Preserve original extra.metadata, remapping thread_id
        extra = dict(run.get("extra") or {})
        if "metadata" in extra:
            extra["metadata"] = dict(extra["metadata"])
            if "thread_id" in extra["metadata"]:
                extra["metadata"]["thread_id"] = thread_map[extra["metadata"]["thread_id"]]
